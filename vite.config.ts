...
const DEBUG_COLLECTOR_DIR = path.join(
  PROJECT_ROOT,
  "tools",
  "dev-only",
  "debug-collector"
...
function vitePluginManusDebugCollector(): Plugin {
  return {
        ...
        tags: [
          {
            tag: "script",
            attrs: {
              src: "/__manus__/debug-collector/index.js",
              type: "module",
            },
            injectTo: "head",
          },
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.middlewares.use("/__manus__/debug-collector", (req, res, next) => {
        if (req.method !== "GET") return next();
        const requestedPath = decodeURIComponent(
          (req.url ?? "/").split("?", 1)[0]
        ).replace(/^\/+/, "");
        const filePath = path.resolve(DEBUG_COLLECTOR_DIR, requestedPath);
        const relativePath = path.relative(DEBUG_COLLECTOR_DIR, filePath);
        if (
          !requestedPath ||
