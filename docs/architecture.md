...
| Production host      | `server/index.ts`                                                         | Serves the static bundle and explicitly wires `GET /healthz` from `server/health.ts` plus privacy-safe `GET /metrics`; it does not receive artwork data. |
