...
Use Node 22 and pnpm. Start from a clean clone, run `corepack enable`, and run `pnpm install`. The normal browser-local studio needs no environment configuration. The development-only storage proxy is optional and recognizes `BUILT_IN_FORGE_API_URL` and `BUILT_IN_FORGE_API_KEY` only when deliberately configured.

Before opening a pull request, run `pnpm format:check`, `pnpm check`, `pnpm test`, and `pnpm build`. New behaviour should include a focused automated test wherever the behaviour can be exercised without a live browser session.
