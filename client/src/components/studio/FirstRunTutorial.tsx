...
export default function FirstRunTutorial({ step, onStart, onSkip, onReplay }: { step: TutorialStep; onStart: () => void; onSkip: () => void; onReplay: () => void }) {
  ...
  if (step === "welcome") return <aside className="tutorial-welcome" role="note" aria-labelledby="tutorial-welcome-title"><div className="tutorial-welcome-card"><span className="tutorial-note-corner" aria-hidden="true" /><span className="tutorial-spark"><Sparkles aria-hidden="true" /></span><span className="eyebrow">A little studio note</span><h2 id="tutorial-welcome-title">Let’s make your first world.</h2><p>Three tiny moves, then it’s all yours: pick a shape, move it, and give it colour.</p><div className="tutorial-welcome-actions"><button className="tutorial-start" onClick={onStart}>Show me how <ArrowRight aria-hidden="true" /></button><button className="tutorial-skip" onClick={onSkip}>I’ll explore myself</button></div></div></aside>;
