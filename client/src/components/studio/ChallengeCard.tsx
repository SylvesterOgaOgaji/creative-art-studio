...
export default function ChallengeCard() {
  ...
  const completedChallengeIds = useStudioStore((state) => state.completedChallengeIds);
  ...
  const handleComplete = () => { completeChallenge(challengeId); setEarnedId(challengeId); playStudioSound("celebrate", soundEnabled, soundVolume); window.setTimeout(() => setEarnedId(null), 1400); };
  ...
  return <><aside className={`challenge-card challenge-${challenge.tint} ${earnedId === challengeId ? "is-earned" : ""}`} aria-label="Creative challenge"><div className="challenge-icon"><Lightbulb aria-hidden="true" /></div><div><span className="eyebrow">Tiny challenge</span><h2>{challenge.title}</h2><p>{challenge.detail}</p><button className={`challenge-complete ${isComplete ? "is-complete" : ""}`} onClick={handleComplete} disabled={isComplete}>{isComplete ? <><Check aria-hidden="true" />Badge earned</> : <><Medal aria-hidden="true" />I made it!</>}</button></div><div className="challenge-actions"><button className="challenge-refresh has-tooltip" data-tooltip="Pick another idea" onClick={nextChallenge} aria-label="Show another creative challenge"><RefreshCw aria-hidden="true" /></button><button className="challenge-dismiss has-tooltip" data-tooltip="Hide challenge" onClick={() => setIsHidden(true)} aria-label="Hide this creative challenge"><X aria-hidden="true" /></button></div></aside><section className="badge-shelf" aria-label="Completed challenge badges"><div><Medal aria-hidden="true" /><span><b>Badge shelf</b><small>{completedChallengeIds.length} of {challenges.length} ideas celebrated</small></span></div><div className="badge-row">{challenges.map((item, index) => <span key={item.title} className={`challenge-badge ${completedChallengeIds.includes(index) ? "is-earned" : ""}`} title={completedChallengeIds.includes(index) ? `${item.title} badge earned` : `${item.title} badge waiting`}><span aria-hidden="true">{completedChallengeIds.includes(index) ? "✦" : "○"}</span><small>{index + 1}</small></span>)}</div></section></>;
}
