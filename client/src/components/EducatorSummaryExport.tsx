...
export default function EducatorSummaryExport() {
  ...
  const download = () => {
    exportAnonymizedProjectSummary({
      savedArtworks,
      galleryFolderCount: galleryFolders.length,
      makerSpotlights,
  ...
  return (
    <section
      className="educator-summary-export"
      aria-labelledby="summary-export-title"
    >
      <div className="summary-export-copy">
        <span className="eyebrow">Local learning record</span>
        <h2 id="summary-export-title">
          Download an anonymous project summary.
        </h2>
        <p>
          Use a small JSON file to understand how this browser’s studio has been
          used—without collecting learner identities or creative content.
        </p>
        <div className="summary-privacy-label">
          <LockKeyhole aria-hidden="true" />
          <span>
            Aggregate counts only. No names, titles, tag labels, images, scene
            coordinates, or raw artwork data.
          </span>
        </div>
      </div>
      <aside className="summary-export-card">
        <FileJson aria-hidden="true" />
        <dl>
          <div>
            <dt>Saved worlds</dt>
            <dd>{savedArtworks.length}</dd>
          </div>
          <div>
            <dt>Built shapes</dt>
            <dd>{objectCount}</dd>
          ...
          <div>
            ...
            <dd>{galleryFolders.length}</dd>
          </div>
        </dl>
        <button onClick={download} disabled={!savedArtworks.length}>
          <FileDown aria-hidden="true" />
          Download anonymous JSON
        </button>
        {savedArtworks.length ? (
          <small>
            <ShieldCheck aria-hidden="true" />
            Prepared only from this browser’s saved worlds.
          </small>
        ) : (
          <small>Add and save a world to prepare a local summary.</small>
        )}
      </aside>
    </section>
  );
