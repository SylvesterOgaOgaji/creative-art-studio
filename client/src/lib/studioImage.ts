...
function drawScenePoster(
  ...
  wash.addColorStop(0.52, "#f7ecd9");
  wash.addColorStop(1, "#dfeaea");
  context.fillStyle = wash;
  context.fillRect(0, 0, width, height);
  context.fillStyle = "rgba(70, 102, 233, .12)";
  context.beginPath();
  context.arc(width * 0.52, height * 0.68, width * 0.42, 0, Math.PI * 2);
  context.fill();
  ...
  objects
    ...
    .forEach(object => {
      const size = Math.max(
        24,
        Math.min(88, 43 * ((object.scale[0] + object.scale[1]) / 2))
      );
      const x = width * 0.5 + object.position[0] * width * 0.085;
      const y =
        height * 0.66 -
        object.position[1] * height * 0.12 +
        object.position[2] * height * 0.028;
      context.save();
      context.translate(x, y);
      context.rotate(object.rotation[2] || object.rotation[1] * 0.18);
      ...
      if (object.type === "cube") {
        context.fillRect(-size * 0.44, -size * 0.44, size * 0.88, size * 0.88);
        context.strokeRect(
          -size * 0.44,
          -size * 0.44,
          size * 0.88,
      ...
      if (object.type === "cone") {
        ...
        context.lineTo(size * 0.54, size * 0.5);
        context.lineTo(-size * 0.54, size * 0.5);
        context.closePath();
        context.fill();
        context.stroke();
      ...
      if (object.type === "torus") {
        ...
        context.stroke();
        context.strokeStyle = "rgba(255,250,239,.8)";
        context.lineWidth = 2;
        context.beginPath();
        context.arc(0, 0, size * 0.56, 0, Math.PI * 2);
        context.stroke();
      }
      if (object.material === "neon") {
...
export function captureStudioImage(
  objects: StudioObject[],
  width = 640,
  height = 420
) {
...
function imageFilename(title: string) {
  const stem =
    (title.trim() || "creative-artwork")
      .replace(/[^a-z0-9]+/gi, "-")
      .replace(/(^-|-$)/g, "")
      .toLowerCase() || "creative-artwork";
...
export function exportSavedArtworkImage(
  title: string,
  objects: StudioObject[]
) {
